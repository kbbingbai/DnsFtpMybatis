...
@Component
public class HdfsFtpToSql implements IFtpToSql {
    ...
    private static final Logger log = LoggerFactory.getLogger(HdfsFtpToSql.class);
    ...
    public Map<String, List<FileList>> pullData(String remoteDirectory,List<FileList> requiredTestData, List<FileList> pullAddList,List<FileList> pullUpList) {
        ...
        try {
            ...
            log.error("HdfsFtpToSql发生异常，pullData方法--listFiles()方法错误，异常e：{}", e);
        }
            ...
            for (FTPFile ftpFile : allFile) {
                if (ftpFile.isFile()) {// 判断是一个文件
                    try {
                        ...
                        log.error("HdfsFtpToSql发生异常，pullData方法--UnsupportedEncodingException，异常e：{}", e);
                    }
                } else {//它是一个文件夹
        ...
        if (this.remoteDirectory.equals(remoteDirectory)) {// 完成了一次任务，就让它存储数据库
            ...
            try {
                ...
                log.error("HdfsFtpToSql发生异常，关闭ftpClient失败：{}", e);
            }
