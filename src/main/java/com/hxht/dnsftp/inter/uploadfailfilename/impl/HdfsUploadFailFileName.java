...
@Component
public class HdfsUploadFailFileName implements IUploadFailFileName {
    ...
    private static final Logger log = LoggerFactory.getLogger(HdfsUploadFailFileName.class);
        ...
        if (failFileList!=null&&failFileList.size() > 0) {
            ...
            if (isExistFlag) {
                try {
                    ...
                    log.error("HdfsUploadFailFileName  uploadFailFileName方法 ftpClient.storeFile异常，异常e：{}", e);
                } finally {
                    if (inputStream != null) {
                        try {
                            inputStream.close();
                        } catch (IOException e) {
                            log.error("HdfsUploadFailFileName  uploadFailFileName方法 关闭流inputStream异常，异常e：{}", e);
                        }
                    }
                    if (ftpClient != null) {
                        try {
                            ...
                            log.error("HdfsUploadFailFileName ftpClient关闭失败，异常e：{}", e);
                        }
                    }
                }
            }
            //删除HDFS上面的数据
            String lastDate = DateUtils.getLastDate();
            deleteHdfsData(failFileList,lastDate);
        }
    }
    ...
    public boolean isExist(FTPClient ftpClient, String pullFailFilesDir) {
        ...
        try {
            ...
            log.error("HdfsUploadFailFileName 发生的异常isExist方法，异常e：{}", e);
            e.printStackTrace();
        }
        return flag;
    }
